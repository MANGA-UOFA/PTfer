...
def gen_feature_sample(data_sample, template):
    feature_sample = {}
    ...
    masked_sentence = parse_template(template.strip(), feature_sample['sub_label'].strip())
    feature_sample['input_sentences'] = [masked_sentence[0]]
    return feature_sample

def load_data(data_path, template, vocab_subset=None):
    all_samples = []
    ...
    for data_sample in raw_samples:
        ...
        feature_sample = gen_feature_sample(data_sample, template)
